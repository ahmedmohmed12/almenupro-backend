...
const LEGACY_PUBLIC_PREFIX = '/menu-images';
...
function resolvePublicImageUrl(imageUrl, origin) {
  ...
  const filenameFromPath = (value) => {
    for (const marker of [
      ...
      if (index >= 0) {
        const tail = value.slice(index + marker.length).split('?')[0].split('#')[0];
  ...
  const filename = filenameFromPath(raw) || (isLocalMenuImageUrl(raw) ? path.basename(raw) : null);
  if (filename) {
    return `${origin}${PUBLIC_PREFIX}/${filename}`;
  }

  if (isExternalCdnUrl(raw)) {
...
function mapItemsForPublicApi(items, origin) {
  return (items || []).map((item) => {
    const imageUrl = resolvePublicImageUrl(item.image_url || item.imageUrl, origin);
    return {
      ...item,
      image_url: imageUrl,
      imageUrl,
