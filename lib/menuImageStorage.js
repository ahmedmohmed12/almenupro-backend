...
function resolvePublicImageUrl(imageUrl, origin) {
  ...
  if (isLocalMenuImageUrl(raw)) {
    const filename = path.basename(raw);
    return `${origin}/menu-images/${encodeURIComponent(filename)}`;
  }
...
function mapItemsForPublicApi(items, origin) {
  return items.map((item) => {
    const resolved = resolvePublicImageUrl(item.image_url || item.imageUrl, origin);
    return {
      ...item,
      image_url: resolved,
      imageUrl: resolved,
