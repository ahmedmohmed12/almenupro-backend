...
async function ensureAutoTranslatedBilingual(raw = {}) {
  ...
  const [{ nameAr, nameEn }, { descriptionAr, descriptionEn }] = await Promise.all([
    nameTask,
    descriptionTask,
  ]);
...
async function ensureAutoTranslatedCategory(raw = {}, existing = {}) {
  ...
  let categoryNameEn = String(
    raw.category_name_en ??
      raw.categoryNameEn ??
      existing.category_name_en ??
      existing.categoryNameEn ??
      '',
  ).trim();
  ...
  if (!categoryNameEn) {
    if (containsArabic(categoryName)) {
      categoryNameEn =
        (await translateText(categoryName, 'ar', 'en')) ||
  ...
  const resolvedEn = categoryNameEn || translateCategoryName(categoryName);
  return {
    category_name: categoryName,
    category_name_en: resolvedEn,
    categoryNameEn: resolvedEn,
  };
}
