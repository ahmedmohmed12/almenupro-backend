...
const { migrateMenuItems, itemNeedsBilingualMigration, translateCategoryName } = require('./bilingualMenu');
...
async function ensureBilingualMenuItems() {
  ...
  const needsMigration = items.some((item) => itemNeedsBilingualMigration(item));
  if (!needsMigration) return { updated: 0, total: items.length };

  const { items: migrated, updated } = migrateMenuItems(items);
  if (updated > 0 || needsMigration) {
    await writeItems(migrated);
  }
  return { updated, total: migrated.length };
}
