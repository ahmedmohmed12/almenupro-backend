...
const { normalizeWhatsappSettings } = require('./whatsappPhone');
const {
...
async function ensureBilingualMenuItemsFast() {
  ...
  return { updated, total: items.length, mode: 'fast' };
}

async function ensureBilingualMenuItemsWithAutoTranslate() {
  const items = await readItems();
  ...
  if (!needsMigration) {
    return ensureBilingualMenuItemsFast();
  }
...
async function initDataStore() {
  ...
  if (usesMongo()) {
    ...
    memory.customers = (await readDoc('customers', memory.customers)).map((customer) =>
      ensureRestaurantId(customer),
    );
    ...
    await ensureDefaultRestaurantSettings();
    return;
  }
...
async function ensureDefaultRestaurantSettings() {
  const map = await readSettingsMap();
  const fileSettings = migrateSettingsShape(loadJson(FILES.settings, {}));
  const fileByRestaurant = fileSettings.byRestaurant || {};
  let changed = false;
  ...
  for (const [restaurantId, fileScoped] of Object.entries(fileByRestaurant)) {
    const current = map.byRestaurant[restaurantId] || defaultSettingsPayload();
    const currentWhatsapp = normalizeWhatsappSettings(current).whatsappNumber;
    const fileWhatsapp = normalizeWhatsappSettings(fileScoped).whatsappNumber;

    if (!currentWhatsapp && fileWhatsapp) {
      map.byRestaurant[restaurantId] = {
        ...current,
        ...normalizeWhatsappSettings(fileScoped),
        workingHours: Array.isArray(fileScoped.workingHours)
          ? fileScoped.workingHours
          : current.workingHours,
        updatedAt: fileScoped.updatedAt || new Date().toISOString(),
      };
      changed = true;
    }
  }
