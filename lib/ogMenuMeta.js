...
function isLikelyDirectImageUrl(raw) {
  ...
  try {
    ...
    if (/\.(png|jpe?g|gif|webp|svg|avif|ico)(\?|$)/i.test(path)) {
      return true;
    }
    if (path.includes('/menu-images/') || path.includes('/api/uploads/menu/')) {
      return true;
    }
    return false;
  } catch (_) {
...
function buildRestaurantOgData(restaurant, options = {}) {
  ...
  const logoCandidate = String(
    restaurant.logoUrl || restaurant.logo_url || '',
  ).trim();
  const logoRaw = isLikelyDirectImageUrl(logoCandidate)
    ? logoCandidate
    : pickFirstMenuItemImage(options.menuItems);
  const logoUrl = resolveAbsoluteAssetUrl(logoRaw, backendOrigin, frontendOrigin);
  const imageIsSecure = logoUrl.startsWith('https://');
...
function buildOgMenuHtml(ogData) {
  ...
  const secureImageTag = ogData.imageIsSecure
...
<head>
  ...
  <meta property="og:image:alt" content="${name}">${secureImageTag}
