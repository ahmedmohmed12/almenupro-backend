...
function readCustomerPromo(customer) {
  if (!customer) {
    return { code: '', discount: 0, used: false };
  }
  return {
    code: String(
...
function customerProfileFromRecord(customer) {
  const { generateWalletPromoCode, readWalletBalance } = require('./walletPromoCodes');
  const promo = readCustomerPromo(customer);
  const restaurantId =
    customer.restaurant_id || customer.restaurantId || '';
  const walletBalance = readWalletBalance(customer);
  const walletPromoCode =
    walletBalance > 0
      ? generateWalletPromoCode(restaurantId, customer.phone, walletBalance)
      : '';
  return {
    ...
    personalPromoCode: promo.used ? '' : promo.code,
    personalPromoDiscount: promo.used ? 0 : promo.discount,
    walletBalance,
    walletPromoCode,
  };
}
...
function normalizeCustomer(raw, id, restaurantId) {
  ...
  const walletBalance =
    Number(raw.walletBalance ?? raw.wallet_balance ?? 0) || 0;
  const walletHistory = Array.isArray(raw.walletHistory)
    ? raw.walletHistory
    : Array.isArray(raw.wallet_history)
      ? raw.wallet_history
      : [];
  const nextOrderDiscount =
    Number(raw.nextOrderDiscount ?? raw.next_order_discount ?? 0) || 0;
  const personalPromoCode = String(
    raw.personalPromoCode ?? raw.personal_promo_code ?? '',
  ).trim();
  const personalPromoDiscount =
    Number(raw.personalPromoDiscount ?? raw.personal_promo_discount ?? 0) || 0;
  ...
  return ensureRestaurantId(
    {
      ...
      walletHistory: walletHistory.slice(0, 50),
      wallet_history: walletHistory.slice(0, 50),
      nextOrderDiscount,
      next_order_discount: nextOrderDiscount,
      personalPromoCode,
      personal_promo_code: personalPromoCode,
      personalPromoDiscount,
      personal_promo_discount: personalPromoDiscount,
      personalPromoUsed:
        raw.personalPromoUsed === true || raw.personal_promo_used === true,
      personal_promo_used:
        raw.personalPromoUsed === true || raw.personal_promo_used === true,
      personalPromoCreatedAt:
        raw.personalPromoCreatedAt || raw.personal_promo_created_at || null,
      personalPromoRedeemedAt:
        raw.personalPromoRedeemedAt || raw.personal_promo_redeemed_at || null,
      welcomeRewardGrantedAt:
        raw.welcomeRewardGrantedAt || raw.welcome_reward_granted_at || null,
  ...
  next[existingIndex] = normalizeCustomer(
    {
      ...
      walletBalance: existing.walletBalance ?? existing.wallet_balance ?? 0,
      walletHistory: existing.walletHistory ?? existing.wallet_history ?? [],
      nextOrderDiscount:
        existing.nextOrderDiscount ?? existing.next_order_discount ?? 0,
      welcomeRewardGrantedAt:
        existing.welcomeRewardGrantedAt ?? existing.welcome_reward_granted_at,
